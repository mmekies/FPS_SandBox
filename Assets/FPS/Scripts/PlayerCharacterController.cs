...
public class PlayerCharacterController : MonoBehaviour
{
    ...
    void GroundCheck()
    {
        ...
        if (Time.time >= m_LastTimeJumped + k_JumpGroundingPreventionTime)
        {
            ...
            if (Physics.CapsuleCast(GetCapsuleBottomHemisphere(), GetCapsuleTopHemisphere(m_Controller.height), m_Controller.radius, Vector3.down, out RaycastHit hit, chosenGroundCheckDistance, groundCheckLayers, QueryTriggerInteraction.Ignore))
            {
                ...
                {
                    // handle material sound
                    if(hit.collider.material.name=="Stone (Instance)" && footstepSFX.name!="Footstep"){
                        //change footstepsfx to stone
                        Debug.Log("STONE");
                        footstepSFX = Resources.Load("Footstep") as AudioClip;
                    }
                    if(hit.collider.material.name=="Metal (Instance)" && footstepSFX.name!="MetalFootstep"){
                        //change footstepsfx to metal
                        Debug.Log("METAL");
                        footstepSFX = Resources.Load("MetalFootstep") as AudioClip;
                    }
                    isGrounded = true;
    ...
    void OnControllerColliderHit(ControllerColliderHit hit) {
        ...
        if(hit.collider.material.name=="Stone"){
              Debug.Log("ON STONE");
        }
        if(hit.collider.material.name=="Metal"){
              Debug.Log("ON METAL");
        }
