...
public class DetectionModuleSoldier : MonoBehaviour
{
    ...
    public float detectionRange = 20f;
     [Tooltip("The max distance at which the enemy can see targets")]
    public float proximityrangemultiplier = 2.5f;
    ...
    {
        EnemyActions = GetComponentInParent<Actions>();
        m_ActorsManager = FindObjectOfType<ActorsManager>();
        DebugUtility.HandleErrorIfNullFindObject<ActorsManager, DetectionModuleSoldier>(m_ActorsManager, this);
    }
    ...
    public virtual void HandleTargetDetection(Actor actor, Collider[] selfColliders)
    {
        ...
        foreach (Actor otherActor in m_ActorsManager.actors)
        {
            if (otherActor.affiliation != actor.affiliation)
            {
                ...
                //add check if within arc of sight
                float angle = Vector3.Angle((otherActor.transform.position - detectionSourcePoint.position), transform.forward);
                float ydist = Vector3.Distance(new Vector3(0,otherActor.transform.position.y,0),new Vector3(0,transform.position.y,0));
                float closeproximity =  (detectionRange/proximityrangemultiplier) * (detectionRange/proximityrangemultiplier);

                //IF TARGET IS WITHIN SIGHT
                if (sqrDistance < sqrDetectionRange && sqrDistance < closestSqrDistance&&angle<90f&&ydist<2||sqrDistance < (sqrDetectionRange/closeproximity) && sqrDistance < closestSqrDistance&&ydist<1)
                {
